...
function WinchLoader()
    ...
    ---Reloads an existing Winch object. For when the a save is loaded.
    ---@param winch Winch Winch to be reloaded
    function Winch.Reload(winch)
        setmetatable(winch, Winch)
    end
    ...
    function Winch:Update(dt)
        if self:HasBothAttachments() then
            ...
            if length_delta > 0 then
                ...
                local impulse = VecScale(dir, m * 0.25 * Constants.UPDATE_FPS * dt)
    ...
    function Winch:Draw(dt)
        if self._attachments[1] then
            ...
            if self._attachments[2] then -- Draw line from target 1 to target 2
                ...
                local hit, dist = QueryRaycast(ct.pos, dir, Constants.PLAYER_RANGE)
                ...
                if hit then -- Voxel in range, preview white line
                    ...
                    DrawLine(p1,
                        VecAdd(ct.pos, QuatRotateVec(ct.rot, { 0, 0, -Constants.PLAYER_RANGE }))
                        , 1, 0, 0)
                end
